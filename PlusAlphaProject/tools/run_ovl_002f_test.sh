...
validate() {
    ...
    if [[ ! -f "$RUNTIME_STATE" ]]; then
        local partial
        partial="$(find "$PROJECT_ROOT/local/overlay" -maxdepth 1 -type d -name 'ovl-002f-test-runtime-*' 2>/dev/null | sort | tail -n 1 || true)"
    ...
    [[ "$(state_value "$RUNTIME_STATE" target)" == 0x80044C7C ]] || fail "Raiz divergente."
    [[ "$(state_value "$RUNTIME_STATE" continuation)" == 0x80044CAC ]] || fail "Retorno JALR divergente."
...
print(f'Runtime OVL-002F validado: {data.get("dll_count",0)} DLL(s), {data.get("unique_entries",0)} entradas.')
PY
}

main() {
    case "${1:-}" in '') ;; -h|--help) printf 'Uso: bash tools/run_ovl_002f_test.sh\n'; return ;; *) fail "Argumento desconhecido." ;; esac
    validate
    local log="$RUNTIME_DIR/ovl-002f-runtime.log"
    printf '\n==> Abrindo o runtime experimental OVL-002F\n'
