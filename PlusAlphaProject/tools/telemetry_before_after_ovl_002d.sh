...
readonly STATE_FILE="$PROJECT_ROOT/local/telemetry/.ovl-002d-test-active.state"
...
usage() {
    cat <<'EOF'
Uso no MSYS2 UCRT64, com a mesma execucao OVL-002D aberta:
...
Alterne bombas com acerto, erro e bloqueio. Nao use outro golpe.
EOF
}
...
read_runtime() {
    ...
    [[ "$(state_value "$RUNTIME_STATE" target)" == 0x80047DE8 ]] || fail "Raiz divergente."
    [[ "$(state_value "$RUNTIME_STATE" incremental_words)" == 69 && "$(state_value "$RUNTIME_STATE" image_body_words)" == 2143 ]] || fail "Volume divergente."
...
if data.get('track')!='OVL-002D-DDARK-47DE8-CUMULATIVE' or int(data.get('image_body_words',0))!=2143:
    raise SystemExit('manifesto nao pertence a OVL-002D')
if data.get('new_root')!='0x80047DE8' or int(data.get('incremental_words',0))!=69: raise SystemExit('manifesto OVL-002D divergente')
...
make_run_dir() {
    ...
    for suffix in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/ovl-002d-telemetry-$suffix"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; RUN_DIR="$candidate"; return; fi
    done
    fail "Nao ha pasta livre para telemetria OVL-002D."
}
...
read_state() {
    ...
    case "$RUN_DIR" in "$PROJECT_ROOT"/local/telemetry/ovl-002d-telemetry-*) ;; *) fail "Run invalido." ;; esac
}
...
live_snapshot() {
    local phase="$1" pc
    for pc in 80047DE8 80049568 80049808 80049988 80092C2C 80093E4C; do
...
expected={'80047DE8':0xBEFA5EA1,'80049568':0x9611F1A0,'80049808':0xCB050F22,'80049988':0x6D2C8CD6,'80092C2C':0x1B64D20F,'80093E4C':0x12FF550F}
def raw(name):
...
print(f'Gate {phase.upper()}: seis PCs possuem CRC vivo e candidato GCC exatos.')
PY
}
...
arm_watch() {
    ...
    local pc
    for pc in 80047DE8 80049568 80049808 80049988 80092C2C 80093E4C; do raw "$RUN_DIR/prepare_pc_watch_arm_${pc}.log" pc_watch_arm target="0x$pc"; done
...
watch=json.loads(match.group(1)); targets={str(x.get('target','')).upper() for x in watch.get('entries',[])}
expected={'0X80047DE8','0X80049568','0X80049808','0X80049988','0X80092C2C','0X80093E4C'}
...
roles={'0X80047DE8':'nova raiz OVL-002D','0X80049568':'sentinela raiz OVL-002C','0X80049808':'sentinela alias OVL-002C 1',
       '0X80049988':'sentinela alias OVL-002C 2','0X80092C2C':'sentinela OVL-002B','0X80093E4C':'sentinela OVL-002A'}
targets={}; errors=[]
for pc,role in roles.items():
    watch_row=seen.get(pc,{}); hits=int(watch_row.get('hits',0) or 0); native=int(watch_row.get('native_hits',0) or 0); interp=int(watch_row.get('interpreted_hits',0) or 0)
...
result={'track':'OVL-002D-DDARK-47DE8-CUMULATIVE','duration_s':round(duration,3),'incremental_words':69,'prior_image_words':2074,'image_body_words':2143,
...
lines=['# OVL-002D - raiz D.Dark 0x80047DE8','',f'- Duracao: {duration:.3f} s','- Incremento: 69 palavras; corpo cumulativo: 2.143',
...
for pc in roles:
    row=targets[pc]; lines.append(f'| `{pc}` | {row["role"]} | {row["native_hits"]} | {row["interpreted_hits"]} |')
...
prepare_phase() {
    [[ ! -f "$STATE_FILE" ]] || fail "Ja existe coleta OVL-002D ativa."
    make_run_dir
...
before_phase() {
    ...
    note "Validando raiz OVL-002D e sentinelas cumulativas"
    ...
    printf '\nBEFORE concluido. Execute 10 bombas, 5 de cada lado, com acerto/erro/bloqueio. Depois execute AFTER.\n'
}

after_phase() {
    read_state; [[ "$RUN_PHASE" == before ]] || fail "AFTER exige fase before."
    note "Coletando gate cumulativo OVL-002D"
...
print('Gate AFTER OVL-002D: nova raiz e cinco sentinelas nativas, sem fallback nos seis PCs.')
PY
