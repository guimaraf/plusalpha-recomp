# build_tele_s1_308.ps1
# Automacao de compilacao da build isolada de telemetria buildTele-s1-308
[CmdletBinding()]
param(
...
Write-Host "     COMPILACAO DA BUILD DE TELEMETRIA - MICRO-LOTE S1-308            " -ForegroundColor Cyan
...
if (-not (Test-Path $SourceCache)) {
    $SourceCache = Join-Path $ProjectRoot "buildTele-s1-306\cache"
}
...
Write-Host " Build de Telemetria S1-308 concluida com SUCESSO!                    " -ForegroundColor Green
