...
readonly STATE_FILE="$PROJECT_ROOT/local/telemetry/.s1-256-telemetry-active.state"
readonly WATCH_SPEC=$'root required 0x8016FC28\nchar_dispatch required 0x801910A4\naux_state required 0x801914C0\noptions_main required 0x80191C84\noptions_leaf required 0x80192D6C\nindirect_selected required 0x801930BC\nindirect_guard_0 guard 0x80192128\nindirect_guard_1 guard 0x80193174\nindirect_guard_3 guard 0x8019319C\nindirect_guard_4 guard 0x801931C4\nindirect_guard_5 guard 0x80192F60\nresume_ram optional_resume 0x80191E70\nresume_table required_resume 0x801920A4\nresume_overlay required_resume 0x8019313C'
...
Uso, sempre no MSYS2 UCRT64 e com a mesma execucao aberta do jogo:
  ...
  bash tools/telemetry_before_after_s1_256.sh prepare
  bash tools/telemetry_before_after_s1_256.sh before
  bash tools/telemetry_before_after_s1_256.sh after

Rota dirigida Options/Memory Card:

  1. Abra manualmente a build S1-256 de telemetria.
  2. No menu principal, destaque Options sem entrar e execute PREPARE.
  3. Ainda no menu, sem inputs, execute BEFORE.
  4. Repita a rota que produziu options-memory-card-complete-clean: entre em
     Options, percorra as telas de Memory Card, conclua as operacoes usadas na
     coleta de descoberta e retorne ao menu principal.
  5. No menu principal, sem inputs, execute AFTER.

O coletor usa pc_watch sem ring finito, registra as duas celulas de JALR e
vigia os cinco destinos indiretos que devem continuar fora do lote. Ele nao
gera, compila, abre nem fecha o jogo.
EOF
}
...
validate_build() {
    ...
    [[ -f "$GAME_TOML" && -f "$RANGES_FILE" && -f "$RAW_TCP" &&
       -f "$CMAKE_CACHE" && -f "$RUNTIME_EXE" ]] ||
        fail "Arquivos da build S1-256 de telemetria estao ausentes."
    ...
    grep -q '^CMAKE_BUILD_TYPE:STRING=RelWithDebInfo$' "$CMAKE_CACHE" ||
        fail "A build S1-256 nao esta RelWithDebInfo."
    grep -q '^PSX_DEBUG_TOOLS:BOOL=ON$' "$CMAKE_CACHE" ||
        fail "A build S1-256 nao possui PSX_DEBUG_TOOLS=ON."
    grep -q '^PSX_STATIC_RUNTIME:BOOL=ON$' "$CMAKE_CACHE" ||
        fail "A build S1-256 nao possui PSX_STATIC_RUNTIME=ON."
...
make_run_dir() {
    ...
    for n in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/s1-256-telemetry-$n"
        if [[ ! -e "$candidate" ]]; then
            mkdir "$candidate"
            RUN_DIR="$candidate"
            return
    ...
    fail "Nao ha run-id livre entre s1-256-telemetry-01 e 99."
}
...
read_state() {
    [[ -f "$STATE_FILE" ]] ||
        fail "Nao existe coleta S1-256 preparada. Execute primeiro prepare."
    ...
    [[ -n "$RUN_DIR" && -d "$RUN_DIR" ]] ||
        fail "Estado S1-256 invalido: run_dir ausente."
    case "$RUN_DIR" in
        "$PROJECT_ROOT"/local/telemetry/s1-256-telemetry-*) ;;
        *) fail "Estado S1-256 fora de local/telemetry." ;;
    esac
    [[ "$RUN_PHASE" == prepared || "$RUN_PHASE" == before ]] ||
        fail "Estado S1-256 invalido: fase $RUN_PHASE."
    [[ "$RUN_START_EPOCH" =~ ^[0-9]+$ ]] ||
        fail "Estado S1-256 invalido: start_epoch."
}
...
candidate=S1-256
functions=0x8016FC28,0x801910A4,0x801914C0,0x80191C84,0x80192D6C,0x801930BC
words=39,141,50,297,59,36; total=622
jalr_cells=0x80020800,0x800D6400
indirect_table=0x801B8538; selected=0x801930BC; guards=0x80192128,0x80193174,0x8019319C,0x801931C4,0x80192F60
ranges_sha256=$(sha256sum "$RANGES_FILE" | awk '{print $1}')
runtime_exe_sha256=$(sha256sum "$RUNTIME_EXE" | awk '{print $1}')
runtime_build=buildClean-ucrt-s1-256-tele
mode=Options/Memory Card
route=BEFORE no menu principal com Options destacado; repetir options-memory-card-complete-clean; retornar ao menu principal; AFTER sem inputs
...
collect_before() {
    note "Coletando BEFORE no menu principal com Options destacado"
...
targets={
    'root':('required','0X8016FC28','raiz formal'),
    'char_dispatch':('required','0X801910A4','dispatch por caractere/tela'),
    'aux_state':('required','0X801914C0','estado auxiliar'),
    'options_main':('required','0X80191C84','corpo principal Options/Memory Card'),
    'options_leaf':('required','0X80192D6C','folha direta'),
    'indirect_selected':('required','0X801930BC','destino indireto observado'),
    'indirect_guard_0':('guard','0X80192128','destino de tabela nao selecionado'),
    'indirect_guard_1':('guard','0X80193174','destino de tabela nao selecionado'),
    'indirect_guard_3':('guard','0X8019319C','destino de tabela nao selecionado'),
    'indirect_guard_4':('guard','0X801931C4','destino de tabela nao selecionado'),
    'indirect_guard_5':('guard','0X80192F60','destino de tabela nao selecionado'),
    'resume_ram':('optional_resume','0X80191E70','retorno do JALR via 0x80020800'),
    'resume_table':('required_resume','0X801920A4','retorno do destino da tabela'),
    'resume_overlay':('required_resume','0X8019313C','retorno do callback via 0x800D6400'),
}
...
def memory_words(name):
    return [str(word).upper() for word in payload(name).get('words',[])]
...
required=[row for row in rows if row[1]=='required']
guards=[row for row in rows if row[1]=='guard']
required_resumes=[row for row in rows if row[1]=='required_resume']
optional_resumes=[row for row in rows if row[1]=='optional_resume']
required_ok=all(row[4]>0 and row[5]==0 for row in required)
guards_clean=all(row[6]==0 for row in guards)
resumes_ok=all(row[4]>0 and row[5]==0 for row in required_resumes)

before_risky=risky_text('before')
after_risky=risky_text('after')
protected={row[2] for row in required+required_resumes}
fallback=sorted(protected & (before_risky | after_risky))
...
handoff_delta=num(adirty.get('native_handoffs'))-num(bdirty.get('native_handoffs'))
miss_delta=num(adisp.get('miss_total'))-num(bdisp.get('miss_total'))
gate=(
    watch_shape and required_ok and guards_clean and resumes_ok and
    observed_clean and not fallback and aborts_stable and blocked_stable and
    miss_delta==0
)
...
before_cell_a=memory_words('before_cell_80020800.log')
after_cell_a=memory_words('after_cell_80020800.log')
before_cell_b=memory_words('before_cell_800D6400.log')
after_cell_b=memory_words('after_cell_800D6400.log')
...
lines += [
    '',
    f'- Seis funcoes obrigatorias nativas sem interpretacao: {"sim" if required_ok else "nao"}',
    f'- Retornos obrigatorios de JALR observados: {"sim" if resumes_ok else "nao"}',
    f'- Retorno opcional 0x80191E70: {optional_resumes[0][6] if optional_resumes else 0} hits',
    f'- Outros cinco destinos indiretos permaneceram zerados: {"sim" if guards_clean else "nao"}',
    f'- PCs protegidos presentes em fallback de texto: {fallback or "nenhum"}',
    f'- Gate tecnico da S1-256: {"confirmado" if gate else "insuficiente"}',
    f'- Delta static_hits: {num(adisp.get("static_hits"))-num(bdisp.get("static_hits"))}',
    f'- Delta miss_total: {miss_delta}',
    '',
    '## JALR e RAM dinamica',
    '',
    f'- 0x80020800 BEFORE/AFTER: {before_cell_a or "n/d"} / {after_cell_a or "n/d"}',
    f'- 0x800D6400 BEFORE/AFTER: {before_cell_b or "n/d"} / {after_cell_b or "n/d"}',
    f'- Delta native_handoffs: {handoff_delta} (diagnostico; a rota possui callbacks em RAM)',
    '',
    ...
    '',
    '- Um native_handoff novo nao reprova sozinho este lote porque as duas funcoes com JALR chamam codigo carregado em RAM; os retornos nativos e os demais gates determinam a integridade.',
...
prepare() {
    ...
    trap cleanup EXIT
    [[ ! -e "$STATE_FILE" ]] ||
        fail "Ja existe uma coleta S1-256 pendente. Conclua com AFTER antes de preparar outra."
    make_run_dir
    metadata
    printf '\nArtefato S1-256 validado; jogo detectado na porta %s.\n' "$DEBUG_PORT"
    printf 'Precondicao: menu principal com Options destacado, ainda sem entrar.\n'
    note "Armando seis funcoes, cinco guards indiretos e tres retornos de JALR"
    arm_instrumentation
    write_state prepared
    PRESERVE_SESSION=1
    note "Preparacao concluida: sem entrar em Options, execute BEFORE no mesmo menu"
}
...
before_phase() {
    ...
    write_state before
    PRESERVE_SESSION=1
    note "BEFORE concluido. Repita a rota Options/Memory Card e volte ao menu principal"
}
