...
readonly TELEMETRY_STATE="$PROJECT_ROOT/local/telemetry/.ovl-002d-test-active.state"
readonly TEST_CONFIG="$PROJECT_ROOT/game_ovl_002d_test.toml"
readonly TARGET_FILE="$PROJECT_ROOT/seeds/ovl_002d_target_pcs.txt"
...
readonly EXPECTED_NEW_SHA=72081505A413F5F2B6E6DEA7162CAC427AC4E977408B13E5364657A366D61E82
readonly EXPECTED_CUMULATIVE_SHA=06358D70C43FBE0E004784C448EA3AEE6ED3AC91AEFFE5BDD66917BCEBF11B56
...
Compila somente o shard F943B63B em runtime isolado. Preserva as 2.074
palavras OVL-002A+B+C e acrescenta apenas as 69 palavras de 0x80047DE8.
...
read_source_state() {
    [[ -f "$SOURCE_STATE" ]] || fail "Runtime OVL-002C ausente."
    ...
    case "$SOURCE_RUNTIME_DIR" in "$PROJECT_ROOT"/local/overlay/ovl-002c-test-runtime-*) ;; *) fail "Estado OVL-002C invalido." ;; esac
    ...
    [[ "$(state_value "$SOURCE_STATE" capture_key)" == "$CAPTURE_KEY" ]] || fail "Chave OVL-002C divergente."
    [[ "$(state_value "$SOURCE_STATE" image_body_words)" == 2074 ]] || fail "Corpo OVL-002C divergente."
}
...
validate_inputs() {
    ...
    [[ ! -f "$TELEMETRY_STATE" ]] || fail "Existe coleta OVL-002D ativa."
    for file in "$SOURCE_RUNTIME_EXE" "$SOURCE_CACHE_MANIFEST" "$SOURCE_CAPTURE_FILE" "$TEST_CONFIG" "$TARGET_FILE" \
                "$COMPILE_OVERLAYS" "$RECOMPILER" "$RUNTIME_INCLUDE/psx_runtime.h" "$RANGES_FILE" "$GCC_BIN"; do
    ...
    "$PYTHON_BIN" - "$SOURCE_RUNTIME_DIR" "$SOURCE_CACHE_MANIFEST" "$SOURCE_CAPTURE_FILE" "$COMPILE_OVERLAYS" "$TEST_CONFIG" "$TARGET_FILE" <<'PY'
...
if manifest.get('track')!='OVL-002C-DDARK-BOMBS-CUMULATIVE' or int(manifest.get('image_body_words',0))!=2074:
    raise SystemExit('cache-base nao e OVL-002C tecnico')
...
walk=co._walk_overlay_function(data,load,len(data),0x80047DE8,0x80047EFC)
visited=set(walk['visited']); raw=b''.join(data[x-load:x-load+4] for x in sorted(visited))
if len(visited)!=69 or min(visited)!=0x80047DE8 or max(visited)!=0x80047EF8:
    raise SystemExit('closure OVL-002D excedeu 69 palavras')
if hashlib.sha256(raw).hexdigest().upper()!='72081505A413F5F2B6E6DEA7162CAC427AC4E977408B13E5364657A366D61E82':
    raise SystemExit('corpo OVL-002D divergente')
if set(walk['jump_table_targets'])!={0x80047E84,0x80047EF4} or walk['direct_jals']:
    raise SystemExit('fluxo/closure OVL-002D divergente')
special={'break':0,'syscall':0,'jalr':0,'cop2':0,'jr_non_ra':0}
for pc in visited:
...
if special!={'break':0,'syscall':0,'jalr':0,'cop2':0,'jr_non_ra':1}: raise SystemExit(f'instrucoes especiais divergiram: {special}')
...
combined=old_words|visited; combined_raw=b''.join(data[x-load:x-load+4] for x in sorted(combined))
if len(old_words)!=2074 or old_words&visited or len(combined)!=2143: raise SystemExit('sobreposicao/volume cumulativo OVL-002D divergente')
if hashlib.sha256(combined_raw).hexdigest().upper()!='06358D70C43FBE0E004784C448EA3AEE6ED3AC91AEFFE5BDD66917BCEBF11B56':
    raise SystemExit('hash cumulativo OVL-002D divergente')
filtered=copy.deepcopy(row)
observed=['0x80092C2C','0x80093E4C','0x80049568','0x80049808','0x80049988','0x80047DE8']
filtered['executed_pcs']=observed; filtered['observed_pcs']=observed[:]
filtered['dispatch_entry_pcs']=observed[:]
filtered['function_entry_pcs']=['0x80092138','0x80092524','0x80092740','0x80092800','0x800929D4','0x80092C2C','0x80093BB8']
filtered['seeds']=observed[:]
...
if reasons.get(0x80047DE8)!='DISPATCH_ENTRY' or len(reasons)!=19: raise SystemExit(f'classificacao OVL-002D divergente: {reasons}')
for alias in (0x80049808,0x80049988,0x80093E4C):
    if reasons.get(alias)!='DISPATCH_INTERIOR': raise SystemExit(f'alias 0x{alias:08X} perdeu classificacao interior')
targets=[int(line.split('#',1)[0].split()[1],16) for line in target_path.read_text().splitlines() if line.split('#',1)[0].split()]
if targets!=[0x80047DE8]: raise SystemExit('target OVL-002D divergente')
print('Pre-auditoria OVL-002D: 69 palavras, zero JAL/JALR/COP2/BREAK/SYSCALL; jump table integralmente interna.')
PY
}
...
make_runtime_dir() {
    ...
    for suffix in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/overlay/ovl-002d-test-runtime-$suffix"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; printf '%s\n' "$candidate"; return; fi
    done
    fail "Nao ha runtime livre para OVL-002D."
}
...
copy_source_runtime() {
    ...
    cp "$SOURCE_CACHE_MANIFEST" "$dst/ovl-002c-cache-source-manifest.json"
}

derive_capture() {
    "$PYTHON_BIN" - "$SOURCE_CAPTURE_FILE" "$1/ovl-002d-filtered-capture.json" <<'PY'
...
row=copy.deepcopy(found[0]); observed=['0x80092C2C','0x80093E4C','0x80049568','0x80049808','0x80049988','0x80047DE8']
...
row['seeds']=observed[:]
row['micro_lot']='OVL-002D cumulative: preserve 2074 words and add only 69-word 0x80047DE8 closure'
output.write_text(json.dumps([row],indent=2)+'\n',encoding='utf-8')
print('Captura privada OVL-002D derivada: 16 roots formais, 3 aliases, 2.143 palavras cumulativas.')
PY
}

compile_shard() {
    local dst="$1" log="$1/ovl-002d-compile.log"
    printf '\n==> Compilando shard cumulativo OVL-002D em %s\n' "$CAPTURE_KEY"
    env -u PSX_OVERLAY_CACHE_DIR -u PSX_OVERLAY_CAPTURES "$PYTHON_BIN" "$COMPILE_OVERLAYS" \
        --captures "$dst/ovl-002d-filtered-capture.json" --capture-key "$CAPTURE_KEY" \
...
source=json.loads((root/'ovl-002c-cache-source-manifest.json').read_text(encoding='utf-8'))
log=(root/'ovl-002d-compile.log').read_text(encoding='utf-8',errors='replace')
if '(0x00020000:0xF943B63B)' not in log or 'Capture filter: 1/' not in log or 'OK ->' not in log: raise SystemExit('ERRO: log nao confirma compilacao filtrada OVL-002D')
if 'SKIP: DLL already exists' in log or re.search(r'(?:RECOMPILER ERROR|GENERATED-C AUDIT FAILED|RANGE AUDIT FAILED|COMPILE ERROR|\bFAILED\b)',log):
    raise SystemExit('ERRO: compilador/auditoria OVL-002D reportou falha')
...
for line in image.read_text(encoding='utf-8').splitlines():
    ...
    if len(p)==3 and p[0]=='R':
        start=int(p[1],16); words.update(range(start,start+int(p[2],16),4))
required={0x00047DE8,0x00049568,0x0004964C,0x000497E4,0x00049808,0x00049988,0x00049B9C,0x00049D24,0x00049E70,0x00049E90,0x00049E9C,
...
forbidden={0x00044830,0x000448EC,0x00044984,0x0004485C,0x00044A68,0x00044AF8,0x00044B8C,0x00047EFC,0x00092F00,0x00094710}
if forbidden&entries: raise SystemExit('candidato fora do microlote entrou no shard')
if len(words)!=2143: raise SystemExit(f'corpo cumulativo tem {len(words)} palavras, esperado 2143')
...
(root/'ovl-002d-cache-manifest.json').write_text(json.dumps({
 'track':'OVL-002D-DDARK-47DE8-CUMULATIVE','capture_keys':['0x00020000:0xAC1FF1A4','0x00020000:0x94E6122F','0x00020000:0xF943B63B'],
 'incremental_words':69,'prior_image_words':2074,'image_body_words':2143,'new_root':'0x80047DE8','new_aliases':[],
 'cop2_incremental':0,'cop2_total_image':115,'new_body_sha256':'72081505A413F5F2B6E6DEA7162CAC427AC4E977408B13E5364657A366D61E82',
 'cumulative_body_sha256':'06358D70C43FBE0E004784C448EA3AEE6ED3AC91AEFFE5BDD66917BCEBF11B56',
...
print(f'Cache OVL-002D auditado: {len(dlls)} DLL(s), {len(all_entries)} entradas e 2.143 palavras na imagem F943B63B.')
PY
}
...
write_state() {
    ...
    {
        printf 'runtime_dir=%s\nruntime_exe=%s\ntest_config=%s\ncache_manifest=%s\n' "$dst" "$dst/StreetFighterEXPlusAlphaRecomp.exe" "$TEST_CONFIG" "$dst/ovl-002d-cache-manifest.json"
        printf 'capture_key=%s\nimage_sha256=%s\ntarget=0x80047DE8\n' "$CAPTURE_KEY" "$EXPECTED_IMAGE_SHA"
        printf 'incremental_words=69\nprior_words=2074\nimage_body_words=2143\ncop2_incremental=0\ncop2_total_image=115\n'
...
main() {
    ...
    printf '\nOVL-002D criada em runtime experimental isolado. O executavel principal nao foi recompilado.\n'
