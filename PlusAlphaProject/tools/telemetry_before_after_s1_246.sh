...
readonly TARGET="0x8011D030"
readonly TARGET_HI="0x8011D078"
readonly HELPER="0x8011D078"
readonly HELPER_HI="0x8011D310"
readonly EXPECTED_CALLER_RA="0x8011720C"
...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/telemetry_before_after_s1_246.sh

Abra manualmente a build S1-246 de telemetria. Em Versus, escolha Doctrine
...
validate_sources() {
    ...
    require_range "F 8011D078"; require_range "R 8011D078 298"
    require_range "F 8011D310"; require_range "F 801171DC"
    require_range "F 80107A74"; require_range "F 80162D68"; require_range "F 80137FE8"; require_range "F 80138084"; require_range "F 8013827C"; require_range "F 801102A0"; require_range "F 8013CB08"
...
for item in items:
    try:
        print(int(json.loads(item).get(field,0) or 0))
        break
...
create_run_directory() {
    ...
    for i in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/s1-246-telemetry-$i"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; RUN_DIR="$candidate"; return; fi
    done
    fail "Nao ha run-id livre entre s1-246-telemetry-01 e 99."
}
...
candidate=S1-246
root_target=$TARGET
root_range=0x8011D030..0x8011D077
helper_target=$HELPER
helper_range=0x8011D078..0x8011D30F
expected_caller_ra=$EXPECTED_CALLER_RA
indirect_dispatcher=0x801171DC
indirect_jalr=0x80117204
dispatch_table_cell=0x801AFC8C
...
collect_before() {
    note "Armando cyc_watch e trace paginado da raiz indireta $TARGET"
