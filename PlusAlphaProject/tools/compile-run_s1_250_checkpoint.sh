#!/usr/bin/env bash
# Checkpoint limpo cumulativo S1-250: Release sem telemetria.
...
readonly BUILD_DIR="$PROJECT_ROOT/buildClean-ucrt-s1-250"
...
readonly EXPECTED_RANGES_SHA256="2a3d4f5fe031ab0ec2404e05b20582654be0dd638d842c63f144b589253e464b"
readonly EXPECTED_FUNCTIONS=1041
...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/compile-run_s1_250_checkpoint.sh

Gera o checkpoint limpo cumulativo S1-250 em Release, com
...
validate_sources() {
    ...
    [[ -f "$RANGES_FILE" ]] || fail "Fontes geradas ausentes: $RANGES_FILE. Execute primeiro o gerador S1-250 no PowerShell."
    [[ -f "$GAME_TOML" && -f "$AUDIT_SCRIPT" ]] || fail "Arquivos de auditoria ausentes para o checkpoint S1-250."
    ...
    require_range "F 80197F88"; require_range "F 8019327C"; require_range "F 8019FB18"; require_range "F 8019FC14"; require_range "F 8019FD3C"
    ! grep -qxF 'F 8019FBD0' "$RANGES_FILE" || fail "A funcao 0x8019FBD0 apareceu fora da closure S1-250."
    ...
    [[ "$function_count" == "$EXPECTED_FUNCTIONS" ]] || fail "Esperadas $EXPECTED_FUNCTIONS funcoes emitidas no checkpoint S1-250; obtidas $function_count."
    ranges_hash="$(sha256sum "$RANGES_FILE" | awk '{print tolower($1)}')"
    [[ "$ranges_hash" == "$EXPECTED_RANGES_SHA256" ]] || fail "Hash dos ranges S1-250 inesperado: $ranges_hash"

    printf '\n==> Auditando fontes gerados S1-250\n'
    python "$AUDIT_SCRIPT" --config "$GAME_TOML"
}

configure_and_build() {
    printf '\n==> Configurando checkpoint limpo S1-250 (Release, PSX_DEBUG_TOOLS=OFF)\n'
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" -G Ninja \
        -DCMAKE_BUILD_TYPE=Release \
        -DPSX_DEBUG_TOOLS=OFF \
        -DPSX_STATIC_RUNTIME=ON
    printf '\n==> Compilando checkpoint limpo S1-250\n'
...
audit_runtime() {
    ...
    require_symbol func_80197F88; require_symbol func_8019327C; require_symbol func_8019FB18; require_symbol func_8019FC14; require_symbol func_8019FD3C
    ! nm -C "$EXE" | grep -Eq '[[:space:]]T[[:space:]]+func_8019FBD0$' || fail "O executavel contem func_8019FBD0 fora da closure S1-250."
