...
$ExpectedBaselineRangesSha256 = "D9A50D9E9DAF597BA6DF3BEB54FC895E1E8E9A6380C0F1095E74E571BF531E68"
$ExpectedBaselineFunctions = 1035
$ExpectedPostFunctions = 1037
$ExpectedSeed = "0x8019F6A8"
$PriorSeeds = @("0x801A9DC0", "0x801A92B8", "0x8011D030", "0x8011D310", "0x80107A74", "0x80162D68", "0x80137FE8", "0x801102A0", "0x8013CB08")
...
$ExpectedNativeTargets = @("F 80197F88", "F 801A92B8")
$ForbiddenFunctions = @("F 8019F5CC", "F 8019FB4C", "F 8019FB84", "F 8019FB94", "F 8019E6D0")
$RawChecks = @(
    @{ Address = "8019F6A8"; Length = 0x1E8; Sha256 = "821C9079D8A6268DE9BC37485ACC312E092415C26944C1816D1EBACBDF5C44FD" },
    @{ Address = "8019FB64"; Length = 0xC; Sha256 = "6403F02E44574F9166B7E8B8D3CB89A7A05B124C57F6427B05647750B30F0D1F" },
    @{ Address = "8019F778"; Length = 0x1C; Sha256 = "3E49A8C3497747BF49EA1F417DE860DF13DC2FAB346F20C79A8825D9660AC08F" },
    @{ Address = "8019F790"; Length = 0x4; Sha256 = "CFA36D040CDD53866BDFD9A6EE138F153FF2BFBA626193E076D6AB13063609D9" }
)
...
foreach ($forbidden in $ForbiddenFunctions) {
    $AlreadyGenerated = $AlreadyGenerated -and (Count-ExactLine $RangesFile $forbidden) -eq 0
}
...
if (-not $AlreadyGenerated) {
    ...
    if ($BaselineHash -ne $ExpectedBaselineRangesSha256) { throw "Ranges antes da geracao nao correspondem ao checkpoint S1-248 validado. SHA-256: $BaselineHash" }
    if ($BaselineFunctions -ne $ExpectedBaselineFunctions) { throw "S1-248 deveria conter $ExpectedBaselineFunctions funcoes; encontrado: $BaselineFunctions" }
    ...
    Write-Host "Checkpoint S1-248 confirmado. Gerando somente as fontes do jogo S1-249..."
    if ($RecompilerPath) { & $GenerateScript -RecompilerPath $RecompilerPath } else { & $GenerateScript }
    if ($LASTEXITCODE -ne 0) { throw "A geracao das fontes S1-249 falhou com codigo $LASTEXITCODE" }
} else { Write-Host "Fontes S1-249 ja presentes; repetindo apenas os gates pos-geracao." }

foreach ($candidate in $CandidateRanges) {
    if ((Count-ExactLine $RangesFile $candidate.Function) -ne 1 -or (Count-ExactLine $RangesFile $candidate.Range) -ne 1) { throw "Range S1-249 ausente ou duplicado: $($candidate.Range)" }
}
foreach ($forbidden in $ForbiddenFunctions) {
    if ((Count-ExactLine $RangesFile $forbidden) -ne 0) { throw "Funcao fora do micro-lote apareceu nos fontes gerados: $forbidden" }
}
$PostFunctions = @(Select-String -LiteralPath $RangesFile -Pattern '^F [0-9A-Fa-f]{8}$').Count
if ($PostFunctions -ne $ExpectedPostFunctions) { throw "S1-249 deveria conter $ExpectedPostFunctions funcoes; encontrado: $PostFunctions" }
...
if ($LASTEXITCODE -ne 0) { throw "A auditoria codegen do S1-249 falhou com codigo $LASTEXITCODE" }
$RangesHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $RangesFile).Hash
Write-Host ""; Write-Host "S1-249 gerado e auditado."; Write-Host "Funcoes: $PostFunctions"; Write-Host "Raiz: 0x8019F6A8 (122 palavras) + thunk BIOS 0x8019FB64 (3 palavras)"; Write-Host "Cobertura esperada: 108.189/195.584 palavras (55,3159%)"; Write-Host "Ranges SHA-256: $RangesHash"; Write-Host "BIOS nao foi regenerada. Nenhum build foi iniciado por este script."
