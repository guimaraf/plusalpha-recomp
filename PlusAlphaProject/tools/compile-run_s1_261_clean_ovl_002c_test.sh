#!/usr/bin/env bash
# Build limpa de diagnostico: EXE estatico S1-261 + cache cumulativo OVL-002C.
...
readonly CLEAN_MANIFEST="$BUILD_DIR/s1-261-ovl-002c-clean-test-manifest.json"
readonly EXPECTED_SOURCE_MANIFEST_SHA256=9FFA51CBC427970CD7E786709578BC1996952837867AF7E0F9A14ABDD8BF996B
...
readonly EXPECTED_CACHE_DLLS=50
readonly EXPECTED_CACHE_ENTRIES=126
readonly EXPECTED_IMAGE_WORDS=2074
readonly EXPECTED_INCREMENTAL_WORDS=966
readonly EXPECTED_PRIOR_IMAGE_WORDS=1108
...
usage() {
    cat <<'EOF'
Uso no MSYS2 UCRT64, com todas as instancias do jogo fechadas:

  bash tools/compile-run_s1_261_clean_ovl_002c_test.sh

Compila buildClean-ucrt-s1-261-ovl-002c-clean-test em Release, sem
telemetria, e copia o cache cumulativo OVL-002C previamente validado.
Nao gera fontes, nao regenera BIOS, nao recompila overlays, nao apaga
sessoes de telemetria e nao abre o jogo.
EOF
}
...
validate_source_tree() {
    ...
    select_python
    [[ -f "$PROJECT_ROOT/CMakeLists.txt" && -f "$GAME_CONFIG" ]] || fail "Projeto/configuracao limpa ausente."
...
validate_overlay_cache() {
    [[ -f "$SOURCE_STATE" ]] || fail "Estado OVL-002C validado ausente: $SOURCE_STATE"
    SOURCE_RUNTIME="$(state_value "$SOURCE_STATE" runtime_dir)"
    SOURCE_MANIFEST="$(state_value "$SOURCE_STATE" cache_manifest)"
    case "$SOURCE_RUNTIME" in "$PROJECT_ROOT"/local/overlay/ovl-002c-test-runtime-*) ;; *) fail "Runtime OVL-002C de origem invalido." ;; esac
    case "$SOURCE_MANIFEST" in "$SOURCE_RUNTIME"/ovl-002c-cache-manifest.json) ;; *) fail "Manifesto OVL-002C de origem invalido." ;; esac
    [[ "$(state_value "$SOURCE_STATE" capture_key)" == 0x00020000:0xF943B63B ]] || fail "Chave OVL-002C divergente."
    [[ "$(state_value "$SOURCE_STATE" image_body_words)" == "$EXPECTED_IMAGE_WORDS" ]] || fail "Corpo OVL-002C divergente."
    [[ "$(state_value "$SOURCE_STATE" incremental_words)" == "$EXPECTED_INCREMENTAL_WORDS" ]] || fail "Incremento OVL-002C divergente."
    [[ -f "$SOURCE_MANIFEST" ]] || fail "Manifesto OVL-002C de origem ausente."
    require_sha256 "$SOURCE_MANIFEST" "$EXPECTED_SOURCE_MANIFEST_SHA256"

    "$PYTHON_BIN" - "$SOURCE_RUNTIME" "$SOURCE_MANIFEST" \
        "$EXPECTED_CACHE_DLLS" "$EXPECTED_CACHE_ENTRIES" "$EXPECTED_IMAGE_WORDS" \
        "$EXPECTED_INCREMENTAL_WORDS" "$EXPECTED_PRIOR_IMAGE_WORDS" <<'PY'
import hashlib,json,pathlib,sys
root=pathlib.Path(sys.argv[1]); manifest=pathlib.Path(sys.argv[2])
dlls,entries,image_words,incremental,prior=map(int,sys.argv[3:])
data=json.loads(manifest.read_text(encoding='utf-8'))
if data.get('track')!='OVL-002C-DDARK-BOMBS-CUMULATIVE': raise SystemExit('manifesto nao pertence a OVL-002C aprovada')
if data.get('capture_keys')!=['0x00020000:0xAC1FF1A4','0x00020000:0x94E6122F','0x00020000:0xF943B63B']:
    raise SystemExit('chaves cumulativas OVL-002C divergentes')
checks=(('dll_count',dlls),('ranges_count',dlls),('unique_entries',entries),
        ('image_body_words',image_words),('incremental_words',incremental),('prior_image_words',prior))
for key,want in checks:
    if int(data.get(key,-1))!=want: raise SystemExit(f'{key} divergente: {data.get(key)} != {want}')
files=data.get('files',[])
if len(files)!=dlls*2: raise SystemExit(f'inventario deveria conter {dlls*2} arquivos; contem {len(files)}')
for row in files:
...
print(f'Cache OVL-002C validado: {dlls} DLLs, {entries} entradas; imagem F943B63B com {image_words} palavras.')
PY
}

configure_and_build() {
    printf '\n==> Configurando build limpa S1-261 + OVL-002C\n'
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" -G Ninja \
        -DCMAKE_BUILD_TYPE=Release \
        -DPSX_DEBUG_TOOLS=OFF \
        -DPSX_STATIC_RUNTIME=ON
    printf '\n==> Compilando buildClean-ucrt-s1-261-ovl-002c-clean-test\n'
...
result={
  'track':'S1-261-CLEAN-TEST-OVL-002C','created_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),
  ...
  'source_cache_track':data['track'],'static_words':111379,'static_total_words':195584,
  'static_coverage_percent':56.9469,'cache_dll_count':int(data['dll_count']),
  'cache_unique_entries':int(data['unique_entries']),'active_image_key':'0x00020000:0xF943B63B',
  'active_image_words':int(data['image_body_words']),'active_image_incremental_words':int(data['incremental_words']),
  'capture_keys':data['capture_keys'],'files':copied,
}
output.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n',encoding='utf-8')
print(f'Cache copiado e revalidado: {result["cache_dll_count"]} DLLs, {result["cache_unique_entries"]} entradas.')
PY
}
...
finish() {
    ...
    printf '\nBUILD LIMPA S1-261 + OVL-002C PRONTA PARA TESTE\n'
    printf '  Build:                    %s\n' "$BUILD_DIR"
    printf '  Executavel:               %s\n' "$EXE"
    printf '  SHA-256 EXE:              %s\n' "$exe_hash"
    printf '  Manifesto local SHA-256:  %s\n' "$manifest_hash"
    printf '  EXE estatico:             111379/195584 palavras (56,9469%%)\n'
    printf '  Cache:                    50 DLLs, 126 entradas\n'
    printf '  Imagem D.Dark F943B63B:   2074 palavras nativas\n'
    printf '  Configuracao:             Release, sem telemetria, sem autocompilacao\n'
    printf '  Sessoes OVL-002C:         preservadas e intocadas\n'
