...
validate() {
    ...
    if [[ ! -f "$RUNTIME_STATE" ]]; then
        local partial
        partial="$(find "$PROJECT_ROOT/local/overlay" -maxdepth 1 -type d -name 'ovl-002c-test-runtime-*' 2>/dev/null | sort | tail -n 1 || true)"
        [[ -z "$partial" ]] || fail "Compilacao OVL-002C incompleta em $partial. Execute novamente compile_ovl_002c_test_runtime.sh."
        fail "Runtime OVL-002C ausente. Execute compile_ovl_002c_test_runtime.sh."
    fi
...
if int(data.get('incremental_words',0))!=966 or int(data.get('prior_image_words',0))!=1108 or int(data.get('image_body_words',0))!=2074:
    raise SystemExit('volumes OVL-002C divergiram')
...
print(f'Runtime OVL-002C validado: {data.get("dll_count",0)} DLL(s), {data.get("unique_entries",0)} entradas.')
PY
}

main() {
    case "${1:-}" in '') ;; -h|--help) printf 'Uso: bash tools/run_ovl_002c_test.sh\n'; return ;; *) fail "Argumento desconhecido." ;; esac
    validate
    local log="$RUNTIME_DIR/ovl-002c-runtime.log"
    printf '\n==> Abrindo o runtime cumulativo OVL-002C\n'
