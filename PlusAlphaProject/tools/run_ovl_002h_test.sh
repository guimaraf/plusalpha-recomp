...
validate() {
    ...
    if [[ ! -f "$RUNTIME_STATE" ]]; then
        local partial
        partial="$(find "$PROJECT_ROOT/local/overlay" -maxdepth 1 -type d -name 'ovl-002h-test-runtime-*' 2>/dev/null | sort | tail -n 1 || true)"
        [[ -z "$partial" ]] || fail "Compilacao OVL-002H incompleta em $partial. Execute novamente compile_ovl_002h_test_runtime.sh."
        fail "Runtime OVL-002H ausente. Execute compile_ovl_002h_test_runtime.sh."
    fi
...
if data.get('track')!='OVL-002H-DDARK-BOMB-SUPER-CUMULATIVE' or int(data.get('image_body_words',0))!=2802 or int(data.get('incremental_words',0))!=239:
    raise SystemExit('manifesto nao pertence a OVL-002H')
...
with socket.socket() as sock:
    ...
    if sock.connect_ex(('127.0.0.1',4535))==0: raise SystemExit('porta 4535 em uso; feche a outra instancia')
print(f'Runtime OVL-002H validado: {data.get("dll_count",0)} DLL(s), {data.get("unique_entries",0)} entradas.')
PY
}

main() {
    case "${1:-}" in '') ;; -h|--help) printf 'Uso: bash tools/run_ovl_002h_test.sh\n'; return ;; *) fail "Argumento desconhecido." ;; esac
    validate
    local log="$RUNTIME_DIR/ovl-002h-runtime.log"
    printf '\n==> Abrindo o runtime experimental OVL-002H\n'
    "$RUNTIME_EXE" --game "$TEST_CONFIG" >"$log" 2>&1 &
    printf 'Jogo iniciado em segundo plano (PID MSYS: %s).\nLog: %s\n' "$!" "$log"
    printf '\nAguarde entrar no jogo. No Mode Select, execute:\n  bash tools/telemetry_before_after_ovl_002h.sh prepare\n'
    printf 'O prepare agora valida a porta 4535 antes de criar qualquer sessao.\n'
}
