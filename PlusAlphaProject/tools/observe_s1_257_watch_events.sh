...
readonly WATCHLIST_FILE="$PROJECT_ROOT/seeds/s1_257_event_watchlist.txt"
...
Uso, no MSYS2 UCRT64, com a build S1-257 de telemetria aberta:
  ...
  bash tools/observe_s1_257_watch_events.sh

Fluxo:

  1. Execute o observador e volte ao jogo.
  2. Percorra menus, transicoes, modos e gameplay normalmente.
  3. Quando uma funcao observada executar, a janela sera congelada e o
...
load_watchlist() {
    ...
    (( ${#WATCH_TARGETS[@]} > 0 )) || fail "Watchlist vazia."
    (( ${#WATCH_TARGETS[@]} <= 128 )) || fail "Watchlist excede 128 enderecos."
}
...
validate() {
    ...
    [[ -f "$RAW_TCP" && -f "$GAME_TOML" && -f "$RANGES_FILE" &&
       -f "$WATCHLIST_FILE" && -f "$CMAKE_CACHE" && -f "$RUNTIME_EXE" ]] ||
        fail "Build S1-257, watchlist ou ferramentas ausentes."
    ...
    nm -C "$RUNTIME_EXE" | grep -E '[[:space:]]T[[:space:]]+func_8019FC6C$' >/dev/null ||
        fail "O executavel nao corresponde a baseline S1-257."
    select_python
    read_debug_port
    load_watchlist
    ...
    for target in "${WATCH_TARGETS[@]}"; do
        ...
        ! grep -q "^F ${address}$" "$RANGES_FILE" ||
            fail "$target ja e nativa na baseline; remova-a da watchlist."
...
make_session_dir() {
    ...
    for n in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/s1-257-watch-events-$n"
        if [[ ! -e "$candidate" ]]; then
...
write_session_metadata() {
    {
        printf 'baseline=S1-257\n'
        printf 'purpose=discovery for future batches\n'
...
lines += ['', '- Baseline executada: S1-257.',
...
main() {
    ...
    write_session_metadata
    arm_watchlist
    note "MONITORANDO ${#WATCH_TARGETS[@]} funcoes na build S1-257"
    printf 'Alvos: %s\n' "${WATCH_TARGETS[*]}"
