...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/compile-run_s1_249_telemetry.sh

Configura e compila a build isolada S1-249 de telemetria. Ao final, confere
a funcao 0x8019F6A8, o thunk BIOS 0x8019FB64 e as DLLs importadas.
...
validate_environment() {
    ...
    [[ -f "$RANGES_FILE" ]] || fail "Ranges S1-249 ausentes: $RANGES_FILE"
...
audit_runtime() {
    ...
    ! nm -C "$EXE" | grep -q -E '[[:space:]]T[[:space:]]+func_8019F5CC$' || fail "O executavel contem o invólucro fora do lote func_8019F5CC."
    ! nm -C "$EXE" | grep -q -E '[[:space:]]T[[:space:]]+func_8019E6D0$' || fail "O executavel contem o candidato em quarentena func_8019E6D0."
    printf '\n==> Runtime validado: S1-249 presente, thunk BIOS nativo e runtime estatico\n%s\n\nExecutavel: %s\n' "$imports" "$EXE"
}
