...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/compile-run_s1_255_telemetry.sh

Configura e compila somente a build isolada S1-255 de telemetria. Nao abre o
...
validate_environment() {
    ...
    [[ -f "$RANGES_FILE" ]] ||
        fail "Fontes S1-255 ausentes. Execute primeiro generate_s1_255_sources.ps1 no PowerShell."
...
configure_and_build() {
    printf '\n==> Configurando S1-255 com telemetria e runtime estatico\n'
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" -G Ninja \
        -DCMAKE_BUILD_TYPE=RelWithDebInfo \
        -DPSX_DEBUG_TOOLS=ON \
        -DPSX_STATIC_RUNTIME=ON
    printf '\n==> Compilando S1-255 de telemetria\n'
...
audit_runtime() {
    ...
    printf '\n==> Runtime S1-255 validado; 0x8018F10C e 1.880 palavras presentes\n%s\n\nExecutavel: %s\nEste script nao abrira o jogo.\n' \
        "$imports" "$EXE"
}
