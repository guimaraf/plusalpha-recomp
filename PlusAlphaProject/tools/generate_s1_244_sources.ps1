...
$ExpectedBaselineRangesSha256 = "8F4282ED07A6956812229ECCB17167BB242AC8F959D3DE7FED0777630F5B420D"
$ExpectedBaselineFunctions = 1029
$ExpectedPostFunctions = 1030
$ExpectedSeed = "0x80107A74"
$PriorSeeds = @("0x80162D68", "0x80137FE8", "0x801102A0", "0x8013CB08")
$QuarantinedSeed = "0x8019E6D0"
$Candidate = @{ Function = "F 80107A74"; Range = "R 80107A74 30C"; Address = "80107A74"; Length = 0x30C; Sha256 = "21AEF47455D2B56FC9177E75E4F9574FC03694BD363C86B45756EEF9851291D7" }
$PriorRanges = @(
...
$ExpectedCallTargets = @("F 801252A8", "F 801690D4", "F 80193A18")
$RawChecks = @(
    @{ Address = "80107A74"; Length = 0x30C; Sha256 = "21AEF47455D2B56FC9177E75E4F9574FC03694BD363C86B45756EEF9851291D7" },
    @{ Address = "8010773C"; Length = 4; Sha256 = "9E4ACA1D882D6BC2EBED00AE07E2D420C1E79CD3A27D4673B23BCD209D3300E7" }
)
...
if (-not $AlreadyGenerated) {
    ...
    if ($BaselineHash -ne $ExpectedBaselineRangesSha256) { throw "Ranges antes da geracao nao correspondem ao S1-243 aprovado. SHA-256: $BaselineHash" }
    if ($BaselineFunctions -ne $ExpectedBaselineFunctions) { throw "S1-243 deveria conter $ExpectedBaselineFunctions funcoes; encontrado: $BaselineFunctions" }
    ...
    Write-Host "Baseline S1-243 confirmada. Gerando somente as fontes do jogo S1-244..."
    if ($RecompilerPath) { & $GenerateScript -RecompilerPath $RecompilerPath } else { & $GenerateScript }
    if ($LASTEXITCODE -ne 0) { throw "A geracao das fontes S1-244 falhou com codigo $LASTEXITCODE" }
} else { Write-Host "Fontes S1-244 ja presentes; repetindo apenas os gates pos-geracao." }
...
if ($PostFunctions -ne $ExpectedPostFunctions) { throw "S1-244 deveria conter $ExpectedPostFunctions funcoes; encontrado: $PostFunctions" }
...
if ($LASTEXITCODE -ne 0) { throw "A auditoria codegen do S1-244 falhou com codigo $LASTEXITCODE" }
$RangesHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $RangesFile).Hash
Write-Host ""; Write-Host "S1-244 gerado e auditado."; Write-Host "Funcoes: $PostFunctions"; Write-Host "Raiz: 0x80107A74 (195 palavras)"; Write-Host "Cobertura esperada: 107.265/195.584 palavras (54,8434%)"; Write-Host "Ranges SHA-256: $RangesHash"; Write-Host "BIOS nao foi regenerada. Nenhum build foi iniciado por este script."
