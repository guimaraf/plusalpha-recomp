...
readonly TARGET="0x801102A0"
readonly TARGET_HI="0x801102A4"
readonly WATCH_MAX=4096
...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/run_s1_241_telemetry.sh
...
O script observa diretamente a entrada nativa 0x801102A0 por cyc_watch e
fntrace. Como a funcao e chamada indiretamente, nao existe gate artificial de
chamador. O script nao compila, abre nem fecha o jogo.
EOF
}
...
validate_sources() {
    ...
    [[ -f "$CMAKE_CACHE" ]] || fail "Cache da build S1-241 ausente: $CMAKE_CACHE"
    [[ -f "$RUNTIME_EXE" ]] || fail "Executavel S1-241 ausente: $RUNTIME_EXE"
    grep -q '^F 801102A0$' "$GENERATED_RANGES" ||
        fail "A funcao 0x801102A0 nao aparece nos fontes. Gere o S1-241 primeiro."
    grep -q '^R 801102A0 390$' "$GENERATED_RANGES" ||
        fail "O range esperado 0x801102A0+0x390 nao aparece nos fontes S1-241."
    ...
    [[ "$(grep -c '^F [0-9A-Fa-f]\{8\}$' "$GENERATED_RANGES")" == "1025" ]] ||
    ...
    grep -q '^CMAKE_BUILD_TYPE:STRING=RelWithDebInfo$' "$CMAKE_CACHE" ||
        fail "A build S1-241 nao esta configurada como RelWithDebInfo."
    grep -q '^PSX_DEBUG_TOOLS:BOOL=ON$' "$CMAKE_CACHE" ||
        fail "A build S1-241 nao possui PSX_DEBUG_TOOLS=ON."
    grep -q '^PSX_STATIC_RUNTIME:BOOL=ON$' "$CMAKE_CACHE" ||
        fail "A build S1-241 nao possui PSX_STATIC_RUNTIME=ON."
    ...
    if printf '%s\n' "$imports" |
       grep -Eqi '^(SDL2\.dll|libgcc_s_seh-1\.dll|libstdc\+\+-6\.dll|libwinpthread-1\.dll)$'; then
        fail "O executavel S1-241 importa uma DLL de runtime nao-sistema."
    fi
    nm -C "$RUNTIME_EXE" | grep -E '[[:space:]]T[[:space:]]+func_801102A0$' >/dev/null ||
        fail "O executavel S1-241 nao contem func_801102A0."
    nm -C "$RUNTIME_EXE" | grep -E '[[:space:]]T[[:space:]]+func_8013CB08$' >/dev/null ||
        fail "O executavel S1-241 perdeu func_8013CB08."
    if nm -C "$RUNTIME_EXE" | grep -E '[[:space:]]T[[:space:]]+func_8019E6D0$' >/dev/null; then
        fail "O executavel S1-241 contem o candidato em quarentena func_8019E6D0."
    fi
...
create_run_directory() {
    ...
    for index in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/s1-241-telemetry-$index"
        if [[ ! -e "$candidate" ]]; then
            mkdir "$candidate"
            RUN_DIR="$candidate"
            return
    ...
    fail "Nao ha run-id livre entre s1-241-telemetry-01 e 99."
}
...
runtime_build=buildClean-ucrt-s1-241-tele
...
target=$TARGET
target_range=0x801102A0..0x8011062F
watch_max=$WATCH_MAX
mode=Versus
...
collect_before() {
    note "Armando cyc_watch direto em 0x801102A0 (capacidade $WATCH_MAX)"
    ...
    note "Armando fntrace direto da entrada S1-241"
...
write_summary() {
    "$PYTHON_BIN" - "$RUN_DIR" "$TARGET" "$WATCH_MAX" <<'PY'
import collections
import json
import pathlib
import re
...
cyc_complete = window_cyc_hits < integer(window_cyc.get("max_hits") or watch_limit)
trace_emitted = integer(trace.get("emitted"))
trace_available = integer(trace.get("available"))
trace_complete = trace_emitted == trace_available and trace_emitted < 4096
...
ra_counts = collections.Counter(str(entry.get("ra", "n/d")) for entry in trace_entries)
a0_counts = collections.Counter(str(entry.get("a0", "n/d")) for entry in trace_entries)
top_ra = ra_counts.most_common(1)[0] if ra_counts else ("n/d", 0)
top_a0 = a0_counts.most_common(1)[0] if a0_counts else ("n/d", 0)
...
lines = [
    ...
    "",
    f"- Entrada: `{target}`",
    "- Range formal: `0x801102A0..0x8011062F` (912 bytes/228 palavras)",
    ...
    f"- Hits fntrace no intervalo: {trace_emitted}",
    f"- Entradas fntrace disponiveis: {trace_available}",
    ...
    f"- Retorno mais frequente: `{top_ra[0]}` ({top_ra[1]} hits)",
    f"- Objeto a0 mais frequente: `{top_a0[0]}` ({top_a0[1]} hits)",
    f"- Prova direta de alcance nativo: {'confirmada' if compiled_proof else 'insuficiente'}",
    f"- Alcance sem fallback: {'confirmado' if compiled_proof and no_candidate_fallback else 'insuficiente'}",
...
collect_telemetry() {
    ...
    printf '\nArtefato S1-241 validado; jogo detectado na porta %s.\n' "$DEBUG_PORT"
    printf 'Precondicao: Doctrine Dark P1 x Skullomania P2, cenario Skullomania; round 1 iniciado e sem inputs.\n'
    printf 'A coleta BEFORE comeca agora e arma diretamente 0x801102A0.\n'
    collect_before
