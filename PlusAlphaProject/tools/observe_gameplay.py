...
Observador de telemetria de alta velocidade para a rota de gameplay e combate:
Selecao de Personagens -> Carregamento da Luta -> Round 1 / Fight -> Combate (golpes, especiais, supers) -> KO / Replay / Vitoria.

Mapeia com precisao todas as instrucoes ainda interpretadas (misses estaticos e hotspots de overlay)
durante o gameplay ativo, identificando os candidatos reais para a proxima etapa de promocao.
"""
...
JOYPAD_SMC_QUARANTINE = {
    0x801AB1F4: "0x801AB1F4 (Quarentena SIO/BIOS Monkey-Patch - Interprete obrigatorio)",
    0x801AB2C0: "0x801AB2C0 (Quarentena BIOS SMC Delay Swap - Interprete obrigatorio)",
}
...
def classify_pc(pc_val, funcs, ranges):
    if pc_val in JOYPAD_SMC_QUARANTINE:
        return f"[QUARANTINE_JOYPAD_SMC] {JOYPAD_SMC_QUARANTINE[pc_val]}"
    if pc_val in funcs:
        return "ENTRY_FUNC (Ja nativo)"
    for lo, hi in ranges:
        if lo <= pc_val < hi:
            return f"CORPO_NATIVO (0x{lo:08X})"
    if 0x80101000 <= pc_val < 0x801C0000:
        return "MAIN_EXE_TEXT (NOVO CANDIDATO GAMEPLAY)"
    return "OVERLAY_DINAMICO (Luta/Personagens)"

def create_run_directory(base_dir, prefix="gameplay-discovery"):
    ...
    for pc in all_miss_pcs:
        ...
        if delta_hits > 0 or delta_misses > 0:
            ...
            static_deltas.append({
                ...
                "is_quarantine": pc_int in JOYPAD_SMC_QUARANTINE,
            })
    static_deltas.sort(key=lambda x: (not x["is_quarantine"], x["delta_selected_hits"]), reverse=True)
...
def main():
    ...
    print("======================================================================")
    print("        OBSERVADOR DE TELEMETRIA: DESCOBERTA DE GAMEPLAY              ")
    print("======================================================================")
    ...
    print("\n" + "=" * 70)
    print("                   PREPARACAO DO SNAPSHOT BEFORE                      ")
    print("=" * 70)
    print("  Instrucoes para isolamento puro de gameplay:")
    print("  1. Navegue ate a tela de Selecao de Personagens ou tela de Versus.")
    print("  2. Quando a tela de luta estiver prestes a comecar (Round 1 / Fight),")
    print("     volte a este terminal e pressione [ENTER].")
    print("  (Isso garante que o BEFORE descarte o historico de menus e meca")
    print("   apenas o que rodar durante o combate)")
    print("=" * 70)

    try:
        input("\n>>> Pressione [ENTER] no inicio do Round 1 para capturar o BEFORE... ")
    ...
    run_dir = create_run_directory(telemetry_base, prefix="gameplay-discovery")
    print(f"[+] Sessao criada em: {run_dir}")

    (run_dir / "before.json").write_text(json.dumps(before, indent=2), encoding="utf-8")

    print("\n" + "=" * 70)
    print("                       GAMEPLAY EM ANDAMENTO                          ")
    print("=" * 70)
    print("  Instrucoes de combate:")
    print("  - Lute normalmente (execute socos, chutes, magias, supers, defesas);")
    print("  - Deixe a luta transcorrer atraves do Round 1 e Round 2;")
    print("  - Ao alcancar a tela de KO, Replay ou Vitoria, volte aqui;")
    print("  - Pressione [ENTER] para capturar o snapshot AFTER.")
    print("=" * 70)

    try:
        input("\n>>> Pressione [ENTER] ao finalizar a luta (tela de KO/Vitoria)... ")
    except KeyboardInterrupt:
    ...
    new_gameplay_candidates = [
        item for item in res["static_text_misses"]
        if "NOVO CANDIDATO GAMEPLAY" in item["classification"]
    ]
    quarantine_items = [
        item for item in res["static_text_misses"]
    ...
    candidates_lines = [
        ...
        "# --- NOVOS CANDIDATOS GAMEPLAY ---",
    ]
    for item in new_gameplay_candidates:
        candidates_lines.append(
            f"{item['class']} {item['pc']} {item['delta_selected_hits']} {item['before_hits']} {item['after_hits']} {item['classification']}"
        )
    if quarantine_items:
        candidates_lines.append("# --- QUARENTENA CONHECIDA (IGNORADA) ---")
    ...
    print(f"\n--- [NOVOS CANDIDATOS GAMEPLAY NO MAIN EXE ({len(new_gameplay_candidates)} encontrados)] ---")
    gameplay_rows = []
    for item in new_gameplay_candidates[:25]:
    ...
    print_table(["PC", "Delta Hits", "Classe", "Diagnóstico"], gameplay_rows)

    if quarantine_items:
        print("\n--- [ITENS EM QUARENTENA IDENTIFICADOS (IGNORADOS)] ---")
        quar_rows = []
        for item in quarantine_items:
            quar_rows.append([
                item["pc"],
        ...
        print_table(["PC", "Delta Hits", "Classe", "Status"], quar_rows)

    print("\n--- [TOP OVERLAY INTERPRETED HOTSPOTS (LUTA)] ---")
    overlay_rows = []
    ...
    summary_lines = [
        "# Telemetria: Descoberta de Gameplay e Combate",
        "",
        ...
        "",
        f"## Novos Candidatos a Promocao Estatica (Main EXE Text): {len(new_gameplay_candidates)}",
        "",
        "| PC | Delta Hits | Classe | Diagnóstico |",
        "|:---|-----------:|:-------|:------------|",
    ]
    for item in new_gameplay_candidates[:40]:
        summary_lines.append(f"| `{item['pc']}` | {item['delta_selected_hits']:,} | {item['class']} | {item['classification']} |")

    if quarantine_items:
        summary_lines.extend([
            "",
            "## Alvos em Quarentena Conhecida (Ignorados)",
            "",
            "| PC | Delta Hits | Status |",
            "|:---|-----------:|:-------|",
        ])
        for item in quarantine_items:
            summary_lines.append(f"| `{item['pc']}` | {item['delta_selected_hits']:,} | `{item['classification']}` |")
    ...
    for item in res["overlay_interp_hot"][:20]:
        summary_lines.append(f"| `{item['pc']}` | {item['delta_insns']:,} | {item['delta_hits']:,} | {item['delta_entry_hits']:,} | {item['classification']} |")

    (run_dir / "summary.md").write_text("\n".join(summary_lines) + "\n", encoding="utf-8")
    print(f"\n[+] Relatorio consolidado salvo em: {run_dir / 'summary.md'}")
    print(f"[+] Lista de candidatos salva em: {run_dir / 'candidates.txt'}")
