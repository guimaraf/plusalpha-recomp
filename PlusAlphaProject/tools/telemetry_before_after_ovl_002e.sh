...
readonly STATE_FILE="$PROJECT_ROOT/local/telemetry/.ovl-002e-test-active.state"
...
readonly WATCH_PCS="80044830 8004485C 800448EC 80044984 80044A68 80044AF8 80044B8C 80047DE8 80049568 80049808 80049988 80092C2C 80093E4C"
...
usage() {
    cat <<'EOF'
Uso no MSYS2 UCRT64, com a mesma execucao OVL-002E aberta:
...
Cubra acerto, erro e bloqueio. Nao misture outro golpe neste gate tecnico.
EOF
}
...
read_runtime() {
    ...
    [[ "$(state_value "$RUNTIME_STATE" target)" == 0x80044830 ]] || fail "Raiz divergente."
    [[ "$(state_value "$RUNTIME_STATE" incremental_words)" == 275 && "$(state_value "$RUNTIME_STATE" image_body_words)" == 2418 ]] || fail "Volume divergente."
...
if data.get('track')!='OVL-002E-DDARK-BOMB-RESIDUAL-CUMULATIVE' or int(data.get('image_body_words',0))!=2418:
    raise SystemExit('manifesto nao pertence a OVL-002E')
if data.get('new_root')!='0x80044830' or len(data.get('new_aliases',[]))!=6: raise SystemExit('manifesto OVL-002E divergente')
...
make_run_dir() {
    ...
    for suffix in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/ovl-002e-telemetry-$suffix"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; RUN_DIR="$candidate"; return; fi
    done
    fail "Nao ha pasta livre para telemetria OVL-002E."
}
...
read_state() {
    ...
    case "$RUN_DIR" in "$PROJECT_ROOT"/local/telemetry/ovl-002e-telemetry-*) ;; *) fail "Run invalido." ;; esac
}
...
expected={'80044830':0x9E9CD462,'8004485C':0x3596728A,'800448EC':0xCD23F9BE,'80044984':0xFE5E309A,
          '80044A68':0xCE5EABF9,'80044AF8':0xB9E07A5F,'80044B8C':0xD7D51FE0,'80047DE8':0xBEFA5EA1,
          '80049568':0x9611F1A0,'80049808':0xCB050F22,'80049988':0x6D2C8CD6,'80092C2C':0x1B64D20F,'80093E4C':0x12FF550F}
def raw(name):
...
print(f'Gate {phase.upper()}: treze PCs possuem CRC vivo e candidato GCC exatos.')
PY
}
...
expected={'0X80044830','0X8004485C','0X800448EC','0X80044984','0X80044A68','0X80044AF8','0X80044B8C','0X80047DE8',
          '0X80049568','0X80049808','0X80049988','0X80092C2C','0X80093E4C'}
...
roles={'0X80044830':'nova raiz OVL-002E','0X8004485C':'alias novo 1','0X800448EC':'alias novo 2','0X80044984':'alias novo 3',
       '0X80044A68':'alias novo 4','0X80044AF8':'alias novo 5','0X80044B8C':'alias novo 6','0X80047DE8':'sentinela OVL-002D',
...
result={'track':'OVL-002E-DDARK-BOMB-RESIDUAL-CUMULATIVE','duration_s':round(duration,3),'incremental_words':275,'prior_image_words':2143,
        'image_body_words':2418,'targets':targets,'loader_deltas':loader,'guard_deltas':guards,'dispatch_miss_delta':miss,'errors':errors,'technical_clean':clean}
(run/'result.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n')
lines=['# OVL-002E - familia residual da bomba','',f'- Duracao: {duration:.3f} s','- Incremento: 275 palavras; corpo cumulativo: 2.418',
...
prepare_phase() {
    [[ ! -f "$STATE_FILE" ]] || fail "Ja existe coleta OVL-002E ativa."
    make_run_dir
...
print('Gate AFTER OVL-002E: familia nova e sentinelas nativas, sem fallback nos treze PCs.')
PY
