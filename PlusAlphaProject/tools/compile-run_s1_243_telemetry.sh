...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/compile-run_s1_243_telemetry.sh

Configura e compila a build isolada S1-243 de telemetria. Ao final, confere
a nova raiz, a baseline e as DLLs importadas. Este script deve ser executado
pelo usuario; ele nao abre o jogo.
EOF
}
...
validate_environment() {
    ...
    [[ -f "$RANGES_FILE" ]] || fail "Ranges S1-243 ausentes: $RANGES_FILE"
    require_range "F 80162D68"; require_range "R 80162D68 3D4"
    require_range "F 80137FE8"; require_range "F 80138084"; require_range "F 8013827C"
    require_range "F 801102A0"; require_range "F 8013CB08"
...
audit_runtime() {
    ...
    printf '\n==> Runtime validado: S1-243 presente, baseline preservada e runtime estatico\n%s\n\nExecutavel: %s\n' "$imports" "$EXE"
}
