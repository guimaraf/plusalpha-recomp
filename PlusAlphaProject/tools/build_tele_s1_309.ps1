# build_tele_s1_309.ps1
# Automacao de compilacao da build isolada de telemetria buildTele-s1-309
[CmdletBinding()]
param(
...
Write-Host "     COMPILACAO DA BUILD DE TELEMETRIA - MICRO-LOTE S1-309            " -ForegroundColor Cyan
...
# Preservar e propagar o cache existente de DLLs do Track 2 (2.140 DLLs)
$SourceCache = Join-Path $ProjectRoot "buildTele-s1-308\cache"
if (-not (Test-Path $SourceCache)) {
    $SourceCache = Join-Path $ProjectRoot "buildTele-s1-307\cache"
}
$DestCache = Join-Path $BuildDir "cache"
if ((Test-Path $SourceCache) -and (-not (Test-Path $DestCache))) {
    Write-Host "[0/4] Propagando cache de overlays do Track 2 (2.140 DLLs)..." -ForegroundColor Green
...
Write-Host " Build de telemetria '$BuildDirName' concluida com SUCESSO!           " -ForegroundColor Green
Write-Host "======================================================================" -ForegroundColor Green
