...
validate() {
    ...
    if [[ ! -f "$RUNTIME_STATE" ]]; then
        local partial
        partial="$(find "$PROJECT_ROOT/local/overlay" -maxdepth 1 -type d -name 'ovl-002e-test-runtime-*' 2>/dev/null | sort | tail -n 1 || true)"
        [[ -z "$partial" ]] || fail "Compilacao OVL-002E incompleta em $partial. Execute novamente compile_ovl_002e_test_runtime.sh."
        fail "Runtime OVL-002E ausente. Execute compile_ovl_002e_test_runtime.sh."
    fi
...
with socket.socket() as sock:
    ...
    if sock.connect_ex(('127.0.0.1',4533))==0: raise SystemExit('porta 4533 em uso; feche a outra instancia')
print(f'Runtime OVL-002E validado: {data.get("dll_count",0)} DLL(s), {data.get("unique_entries",0)} entradas.')
PY
}

main() {
    case "${1:-}" in '') ;; -h|--help) printf 'Uso: bash tools/run_ovl_002e_test.sh\n'; return ;; *) fail "Argumento desconhecido." ;; esac
    validate
    local log="$RUNTIME_DIR/ovl-002e-runtime.log"
    printf '\n==> Abrindo o runtime experimental OVL-002E\n'
