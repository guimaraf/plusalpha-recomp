...
Uso, sempre no MSYS2 UCRT64:
  ...
  bash tools/compile-run_s1_245_telemetry.sh

Configura e compila a build isolada S1-245 de telemetria. Ao final, confere
a nova raiz, a baseline e as DLLs importadas. Este script nao abre o jogo.
EOF
}
...
validate_environment() {
    ...
    [[ -f "$RANGES_FILE" ]] || fail "Ranges S1-245 ausentes: $RANGES_FILE"
    require_range "F 8011D310"; require_range "R 8011D310 6A4"
...
audit_runtime() {
    ...
    printf '\n==> Runtime validado: S1-245 presente, baseline preservada e runtime estatico\n%s\n\nExecutavel: %s\n' "$imports" "$EXE"
}
