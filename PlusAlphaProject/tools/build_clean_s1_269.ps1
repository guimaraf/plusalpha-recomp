# build_clean_s1_269.ps1
# Automacao de compilacao da build isolada limpa (clean) buildClean-ucrt-s1-269
[CmdletBinding()]
param(
    [string]$BuildDirName = "buildClean-ucrt-s1-269",
...
Write-Host "     COMPILACAO DA BUILD LIMPA (CLEAN) - MICRO-LOTE S1-269            " -ForegroundColor Cyan
Write-Host "  Alvo:                 $BuildDirName" -ForegroundColor Yellow
Write-Host "  Perfil:               RelWithDebInfo (-O2 -g -DNDEBUG)" -ForegroundColor Yellow
Write-Host "  Telemetria / Debug:   PSX_DEBUG_TOOLS=OFF (Desativadas para 60 FPS fixos)" -ForegroundColor Yellow
...
# 1. Configurar CMake
Write-Host "[1/4] Configurando diretorio de build limpo com CMake Ninja..." -ForegroundColor Green
$cmakeArgs = @(
    "-B", $BuildDir,
    "-S", $ProjectRoot,
    "-G", "Ninja",
...
if (-not (Test-Path $TargetExe)) {
    throw "Executavel nao encontrado em: $TargetExe"
}
...
Write-Host "  [+] BUILD LIMPA S1-269 GERADA COM SUCESSO!" -ForegroundColor Green
