#!/usr/bin/env bash
# Janela isolada de 30 s para medir o residuo apos OVL-002G.
...
validate() {
    ...
    for suffix in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/ovl-002g-ddark-bombs-discovery-$suffix"
...
print('\nRuntime OVL-002G confirmado. D.Dark P1 x Ryu P2; mantenha Ryu parado.',flush=True)
...
for pc in sorted(native_group):
    ...
    if pc in required_group and native<=0: fatal.append(f'0x{pc:08X} nao teve hit nativo')
...
if sum(group[f'0x{pc:08X}']['native_hits'] for pc in {0x80045E70,0x80045F00,0x80045F80,0x80045F94,0x80045FF8})<=0:
    fatal.append('nenhum alvo da tabela local executou nativo')
...
result={'track':'OVL-002G-DDARK-BOMBS-DISCOVERY','duration_s':elapsed,'pcs':len(rows),'interpreted_insns':total,
...
lines=['# OVL-002G - bombas isoladas','',f'- Duracao: {elapsed:.3f} s',f'- PCs interpretados: {len(rows)}',
...
main() {
    case "${1:-}" in '') ;; -h|--help) printf 'Uso: bash tools/observer_ddark_bombs_ovl_002g.sh\n'; return ;; *) fail "Argumento desconhecido." ;; esac
    validate
    {
        printf 'track=OVL-002G-DDARK-BOMBS-DISCOVERY\nruntime_dir=%s\n' "$RUNTIME_DIR"
        printf 'prepare_seconds=5\nduration_seconds=30\nroute=D.Dark P1 x Ryu P2; cenario Ryu; somente bombas sem acerto\n'
        printf 'baseline_ovl_002f_interpreted_insns=4531472\nbaseline_45e30_family_interpreted_insns=11468\nstarted_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
