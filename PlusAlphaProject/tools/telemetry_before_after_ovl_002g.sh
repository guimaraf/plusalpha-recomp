...
readonly STATE_FILE="$PROJECT_ROOT/local/telemetry/.ovl-002g-test-active.state"
...
readonly REQUIRED_NATIVE_PCS="80045E30 80045EB4 80045EDC 80045F00 80045F38 80046020 80044C7C 80044CAC 80044830 80044B8C 80047DE8 80049568 80049808 80049988 80092C2C 80093E4C"
readonly OPTIONAL_NATIVE_PCS="80045E70 80045F80 80045F94 80045FF8"
readonly NATIVE_PCS="$REQUIRED_NATIVE_PCS $OPTIONAL_NATIVE_PCS"
readonly EXCLUDED_PCS="80044CBC 80045124 80045868 80045A1C 80045C14 80046034 800465A8 80046998 80046C38 80047000"
...
usage() {
    cat <<'EOF'
Uso no MSYS2 UCRT64, com a mesma execucao OVL-002G aberta:
...
Para liberar uma coleta que terminou em REVIEW, preservando seus arquivos:
  ...
  bash tools/telemetry_before_after_ovl_002g.sh retry
...
read_runtime() {
    if [[ ! -f "$RUNTIME_STATE" ]]; then
        ...
        for candidate in "$PROJECT_ROOT"/local/overlay/ovl-002g-test-runtime-*; do
    ...
    [[ "$(state_value "$RUNTIME_STATE" target)" == 0x80045E30 ]] || fail "Raiz OVL-002G divergente."
    [[ "$(state_value "$RUNTIME_STATE" alias_1)" == 0x80045E70 && "$(state_value "$RUNTIME_STATE" alias_9)" == 0x80046020 ]] || fail "Aliases OVL-002G divergentes."
...
if data.get('new_root')!='0x80045E30' or data.get('new_aliases')!=aliases or data.get('jump_table_pc')!='0x80045E68': raise SystemExit('manifesto OVL-002G divergente')
...
make_run_dir() {
    ...
    for suffix in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/ovl-002g-telemetry-$suffix"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; RUN_DIR="$candidate"; return; fi
    done
    fail "Nao ha pasta livre para telemetria OVL-002G."
}
...
read_state() {
    ...
    case "$RUN_DIR" in "$PROJECT_ROOT"/local/telemetry/ovl-002g-telemetry-*) ;; *) fail "Run invalido." ;; esac
}
...
archive_review_phase() {
    ...
    case "$review_dir" in "$PROJECT_ROOT"/local/telemetry/ovl-002g-telemetry-*) ;; *) fail "Run ativo invalido." ;; esac
...
live_snapshot() {
    ...
    raw "$RUN_DIR/${phase}_local_jump_table.log" mem_words addr=0x8004A5C0 count=5
}
...
expected={'80045E30':0x95AB7D43,'80045E70':0x17150306,'80045EB4':0x87E45D08,'80045EDC':0x130F0F62,
          '80045F00':0x7291F211,'80045F38':0x2DF55CC3,'80045F80':0xA8F4C52C,'80045F94':0xC6AEDCB3,
          '80045FF8':0x97ED45B7,'80046020':0xB30CC5EF,'80044C7C':0xD929A55E,'80044CAC':0x990698B6,
          '80044830':0x9E9CD462,'80044B8C':0xD7D51FE0,
...
print(f'Gate {phase.upper()}: handler, aliases, sentinelas e tabelas possuem identidade exata.')
PY
}
...
expected={f'0X{x:08X}' for x in (0x80045E30,0x80045E70,0x80045EB4,0x80045EDC,0x80045F00,0x80045F38,0x80045F80,
          0x80045F94,0x80045FF8,0x80046020,0x80044C7C,0x80044CAC,0x80044830,0x80044B8C,0x80047DE8,0x80049568,
          0x80049808,0x80049988,0x80092C2C,0x80093E4C,0x80044CBC,0x80045124,0x80045868,0x80045A1C,0x80045C14,
...
required_roles={'0X80045E30':'novo handler 6','0X80045EB4':'alias observado 1','0X80045EDC':'alias observado 2',
 '0X80045F00':'alias observado/tabela','0X80045F38':'alias observado 3','0X80046020':'alias observado 4',
 '0X80044C7C':'sentinela raiz OVL-002F','0X80044CAC':'sentinela retorno OVL-002F',
...
optional_roles={'0X80045E70':'alias de tabela 0','0X80045F80':'alias de tabela 2',
 '0X80045F94':'alias de tabela 3','0X80045FF8':'alias de tabela 4'}
native_roles={**required_roles,**optional_roles}
excluded_roles={'0X80044CBC':'handler externo 0','0X80045124':'handler externo 1','0X80045868':'handler externo 2','0X80045A1C':'handlers externos 3/4',
 '0X80045C14':'handler externo 5','0X80046034':'handler externo 7','0X800465A8':'handler externo 8',
 '0X80046998':'handler 10','0X80046C38':'handler 9','0X80047000':'handler 11'}
targets={}; errors=[]
for pc,role in {**native_roles,**excluded_roles}.items():
    ...
    if pc in native_roles:
        if pc in required_roles and native<=0: errors.append(f'{pc}: nenhum hit nativo')
        if interp: errors.append(f'{pc}: {interp} hit(s) interpretado(s)')
    elif native:
        errors.append(f'{pc}: handler excluido teve {native} hit(s) nativo(s)')
jump_aliases=('0X80045E70','0X80045F00','0X80045F80','0X80045F94','0X80045FF8')
if sum(targets[pc]['native_hits'] for pc in jump_aliases)<=0: errors.append('nenhum alvo da tabela local executou nativo')
...
if (before.get('prefix_crc32')!=after.get('prefix_crc32') or before.get('external_table')!=after.get('external_table') or
        before.get('local_jump_table')!=after.get('local_jump_table')):
    errors.append('bytes vivos ou tabelas mudaram')
duration=max(0,(time.time_ns()-start)/1e9); clean=not errors
result={'track':'OVL-002G-DDARK-BOMB-HANDLER-CUMULATIVE','duration_s':round(duration,3),'incremental_words':129,
        'prior_image_words':2434,'image_body_words':2563,'jump_table_pc':'0x80045E68','jump_table_addr':'0x8004A5C0',
...
lines=['# OVL-002G - handler da bomba','',f'- Duracao: {duration:.3f} s','- Incremento: 129 palavras; corpo cumulativo: 2.563',
       ...
       '|---|---|---:|---:|']
for pc in [*native_roles,*excluded_roles]:
...
prepare_phase() {
    [[ ! -f "$STATE_FILE" ]] || fail "Ja existe coleta OVL-002G ativa."
    make_run_dir
...
print('Gate AFTER OVL-002G: handler e aliases nativos; demais handlers permaneceram fora do lote.')
PY
    )"; then
