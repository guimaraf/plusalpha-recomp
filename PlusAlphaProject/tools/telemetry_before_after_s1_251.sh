...
readonly TARGET=0x8014C708
readonly TARGET_HI=0x8014C730
readonly JALR_RA=0x8014C720
readonly POINTER_CELL=0x80018800
readonly REGISTRATION_CELL=0x801B1BB4
readonly PAGE_SIZE=2048
readonly FN_RING_CAP=262144
readonly TRACE_MAX=4096
readonly STATE_FILE="$PROJECT_ROOT/local/telemetry/.s1-251-telemetry-active.state"
...
Uso, sempre no MSYS2 UCRT64 e com a mesma execucao aberta do jogo:
  ...
  bash tools/telemetry_before_after_s1_251.sh prepare
  bash tools/telemetry_before_after_s1_251.sh before
  bash tools/telemetry_before_after_s1_251.sh after

Fluxo:

  1. Abra manualmente a build S1-251 de telemetria.
  2. No menu de modos, com Bonus Barril destacado e antes de escolher Guile,
     execute "prepare".
  3. Escolha Guile. Assim que o cenario estiver ativo, sem inputs, execute
     "before".
...
validate_build() {
    ...
    [[ -f "$GAME_TOML" && -f "$RANGES_FILE" && -f "$RAW_TCP" && -f "$CMAKE_CACHE" && -f "$RUNTIME_EXE" ]] || fail "Arquivos da build S1-251 de telemetria estao ausentes."
    require_range "F 8014C708"; require_range "R 8014C708 28"
    require_range "F 8019F5CC"; require_range "F 8019F6A8"
    ...
    grep -q '^CMAKE_BUILD_TYPE:STRING=RelWithDebInfo$' "$CMAKE_CACHE" || fail "A build S1-251 nao esta configurada como RelWithDebInfo."
    grep -q '^PSX_DEBUG_TOOLS:BOOL=ON$' "$CMAKE_CACHE" || fail "A build S1-251 nao possui PSX_DEBUG_TOOLS=ON."
    grep -q '^PSX_STATIC_RUNTIME:BOOL=ON$' "$CMAKE_CACHE" || fail "A build S1-251 nao possui PSX_STATIC_RUNTIME=ON."
    local imports
    ...
    ! nm -C "$RUNTIME_EXE" | grep -q -E '[[:space:]]T[[:space:]]+func_(80103384|8016FC28|8017566C|8019E6D0)$' || fail "O executavel contem uma funcao fora do micro-lote."
    select_python
    read_debug_port
}
...
for row in rows:
    try:
        data=json.loads(row); raw=bytes.fromhex(str(data.get('hex','')))
        if data.get('ok') and len(raw)==4:
            print(f'0x{struct.unpack("<I",raw)[0]:08X}'); break
    except (ValueError,TypeError,json.JSONDecodeError,struct.error): pass
...
traceable_target() { [[ "$1" =~ ^0x80[01][0-9A-F]{4}[048C]$ ]]; }
...
make_run_dir() {
    ...
    for n in $(seq -w 1 99); do
        candidate="$PROJECT_ROOT/local/telemetry/s1-251-telemetry-$n"
        if [[ ! -e "$candidate" ]]; then mkdir "$candidate"; RUN_DIR="$candidate"; return; fi
    done
    fail "Nao ha run-id livre entre s1-251-telemetry-01 e 99."
}
...
read_state() {
    [[ -f "$STATE_FILE" ]] || fail "Nao existe coleta S1-251 preparada. Execute primeiro: bash tools/telemetry_before_after_s1_251.sh prepare"
    ...
    [[ -n "$RUN_DIR" && -d "$RUN_DIR" ]] || fail "Estado da coleta S1-251 invalido: run_dir ausente."
    case "$RUN_DIR" in "$PROJECT_ROOT"/local/telemetry/s1-251-telemetry-*) ;; *) fail "Estado da coleta S1-251 fora de local/telemetry." ;; esac
    [[ "$RUN_PHASE" == prepared || "$RUN_PHASE" == before ]] || fail "Estado da coleta S1-251 invalido: fase '$RUN_PHASE'."
    [[ "$RUN_START_EPOCH" =~ ^[0-9]+$ ]] || fail "Estado da coleta S1-251 invalido: start_epoch."
    [[ "$RUN_POINTER_TARGET" =~ ^0x[0-9A-F]{8}$ ]] || fail "Estado da coleta S1-251 invalido: pointer_target."
}
...
candidate=S1-251
root_target=$TARGET
root_range=0x8014C708..0x8014C72F
registration_cell=$REGISTRATION_CELL
jalr_site=0x8014C718
...
runtime_build=buildClean-ucrt-s1-251-tele
mode=Bonus Barril
character=Guile
...
collect_before() {
    ...
    current_target="$(pointer_target "$RUN_DIR/before_pointer_cell.log")"
    ...
    dump_traces before "$RUN_POINTER_TARGET"
    raw "$RUN_DIR/window_fn_clear.log" fn_clear
    RUN_POINTER_TARGET="$current_target"
    arm_traces window "$RUN_POINTER_TARGET"
}
...
collect_after() {
    ...
    dump_traces after "$RUN_POINTER_TARGET"
...
summary() {
    "$PYTHON_BIN" - "$RUN_DIR" "$TARGET" "$JALR_RA" "$FN_RING_CAP" <<'PY'
import collections,json,pathlib,re,struct,sys
run,target,jalr_ra,cap=pathlib.Path(sys.argv[1]),sys.argv[2].upper(),sys.argv[3].upper(),int(sys.argv[4])
def payload(name):
    p=run/name
...
root_trace=payload('before_fntrace_root.log').get('entries',[])+payload('after_fntrace_root.log').get('entries',[])
callback_rows=payload('before_fntrace_callback.log').get('entries',[])+payload('after_fntrace_callback.log').get('entries',[])
callback_ras=collections.Counter(str(e.get('ra','')).upper() for e in callback_rows)
callback_targets=collections.Counter(str(e.get('target','')).upper() for e in callback_rows)
prepare_ptr,before_ptr,after_ptr=pointer('prepare_pointer_cell.log'),pointer('before_pointer_cell.log'),pointer('after_pointer_cell.log')
...
pointer_stable=before_ptr != '0x00000000' and before_ptr == after_ptr
jalr_proof=bool(callback_rows) and callback_ras[jalr_ra] == len(callback_rows) and callback_targets[before_ptr.upper()] == len(callback_rows) and pointer_stable
gate=root_proof and jalr_proof
latency=payload('after_latency.log'); phase=payload('after_phase_profile.log'); frame=latency.get('summary',{}).get('frame_period',{})
lines=[f'# Telemetria {run.name}','', '## Resultado S1-251','',
...
f'- RAs observadas da raiz: {root_ras.most_common(8)}',
f'- Hits fntrace da raiz: {len(root_trace)}',
f'- Raiz nativa alcancada sem fallback: {"confirmado" if root_proof else "insuficiente"}',
'', '## JALR dinamico','',
f'- Celula 0x80018800 em prepare/BEFORE/AFTER: {prepare_ptr} / {before_ptr} / {after_ptr}',
f'- fntrace do alvo apontado: {len(callback_rows)} hits; alvo esperado {before_ptr}: {callback_targets[before_ptr.upper()]}; RA esperada {jalr_ra}: {callback_ras[jalr_ra]}; RAs={callback_ras.most_common(5)}',
f'- Ponteiro estavel entre BEFORE e AFTER: {"sim" if pointer_stable else "nao"}',
f'- Gate raiz + retorno dinamico do JALR: {"confirmado" if gate else "insuficiente"}',
...
prepare() {
    validate_build
    trap cleanup EXIT
    [[ ! -e "$STATE_FILE" ]] || fail "Ja existe uma coleta S1-251 pendente. Conclua com AFTER antes de preparar outra."
    make_run_dir
    metadata
    printf '\nArtefato S1-251 validado; jogo detectado na porta %s.\n' "$DEBUG_PORT"
    printf 'Precondicao: menu de modos com Bonus Barril destacado; Guile ainda nao escolhido.\n'
    note "Armando raiz e observador da celula 0x80018800"
    ...
    arm_traces prepare "$RUN_POINTER_TARGET"
    write_state prepared
    PRESERVE_SESSION=1
    note "Preparacao concluida: escolha Guile, entre no Bonus e execute BEFORE com o cenario ativo"
}
...
before_phase() {
    ...
    printf '\nInstrumentacao S1-251 mantida desde o menu. Coletando BEFORE no cenario Bonus ativo.\n'
    collect_before
...
after_phase() {
    ...
    trap cleanup EXIT
    read_state
    [[ "$RUN_PHASE" == before ]] || fail "A coleta ainda esta na fase '$RUN_PHASE'; execute BEFORE dentro do Bonus."
    local end seconds
